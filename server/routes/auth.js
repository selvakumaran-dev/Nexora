...
router.post('/register', validate(registerSchema), async (req, res) => {
    try {
        ...
        if (existingUser) {
            // Generate a unique email with 4-digit random number
            let isUnique = false
            let attempts = 0
            ...
            while (!isUnique && attempts < maxAttempts) {
                ...
                finalEmail = `${emailParts[0]}${randomNum}@${emailParts[1]}`

                const checkUser = await User.findOne({ email: finalEmail })
                if (!checkUser) {
                    isUnique = true
                }
                attempts++
            ...
            if (!isUnique) {
                return res.status(400).json({
                    success: false,
                    message: 'Could not generate unique email. Please try a different email.'
                })
            }
        }

        // Create user with final email
